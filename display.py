...
rectangle = plt.Rectangle((0,0),60,60, fc =  None, ec = 'black', fill = False)
ax.add_patch(rectangle)
rectangle = plt.Rectangle((1,1),60 -2,60 - 2, fc =  None, ec = 'black', fill = False)
ax.add_patch(rectangle)
...
#3 rectangles
origin = tuple(np.add(np.array(origin), np.array((10,10))).tolist())
width = width-wstep
rect_big = plt.Rectangle(origin,40,40, fc = None, ec = 'black', fill = False)
...
corner = tuple(np.add(np.array(origin), np.array((0,width))).tolist())
corners.append(corner)
rect_mid = ptch.Rectangle(origin,30,30, fc = None, ec = 'black', fill = False)
...
corner = tuple(np.add(np.array(origin), np.array((0,width))).tolist())
corners.append(corner)
rect_small = ptch.Rectangle(origin,20,20, fc =  None, ec = 'black', fill = False)
